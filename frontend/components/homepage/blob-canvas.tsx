...
// Reviveo palette — blue that reads on slate-950, with soft white distort
const REVIVEO_BLUE = '#3E7AEE'; // from your original footer logo fill
const REVIVEO_BLUE_DEEP = '#2563eb';
  ...
  useFrame((state) => {
    ...
    if (mesh.current) {
      ...
      material.uniforms.u_intensity.value = MathUtils.lerp(
        ...
        hover.current ? 0.68 : 0.42,
        0.02
      );
      // subtle mouse follow — scaled down for footer, not full screen
      targetPosition.current.set(mouse.x * 0.9, mouse.y * 0.6, 0);
...
export const BlobCanvas: React.FC<{ className?: string; color?: string }> = ({
  className,
  color = REVIVEO_BLUE,
}) => {
  return (
